...
//
// Google Sheet: AppFeedback!A2:L1000  (ต้องสร้าง sheet/tab ชื่อ "AppFeedback" ก่อนใช้งาน
// พร้อมหัวตาราง แถวที่ 1: id | username | requesterName | role | date |
//   q1_easy | q2_speed | q3_design | q4_accuracy | q5_stability | q6_overall | comment)
// A id | B username | C requesterName | D role | E date
// F q1_easy (ใช้ง่าย) | G q2_speed (เร็ว) | H q3_design (หน้าตา) | I q4_accuracy (ถูกต้อง)
// J q5_stability (เสถียร) | K q6_overall (โดยรวม) | L comment
// ─────────────────────────────────────────────────────────────
...
const RANGE = 'AppFeedback!A2:L1000';
...
// body: { username, requesterName, role, q1, q2, q3, q4, q5, q6, comment }
router.post('/', requireAuth, async (req, res) => {
  try {
    const { username, requesterName, role, q1, q2, q3, q4, q5, q6, comment } = req.body;
    ...
    await sheets.spreadsheets.values.append({
      ...
      requestBody: {
        values: [[
          ...
          scores[0], scores[1], scores[2], scores[3], scores[4], scores[5], comment || '',
        ]],
      },
    });
