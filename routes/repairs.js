...
function parseThaiDateTime(str) {
  if (!str) return null;
  try {
    // บั๊กเดิม: โค้ดคาดว่าฟอร์แมตมี comma คั่นวันที่กับเวลา เช่น "22/7/2569, 18:48:30"
    // แต่ .toLocaleString('th-TH', {...}) ของจริงไม่มี comma เลย ("22/7/2569 18:48:30")
    // ทำให้ split(',') ไม่เจอ เวลาเลยหายไปหมด กลายเป็น 00:00:00 ทุกครั้ง — แก้ด้วย regex
    // ที่รองรับทั้งแบบมี comma และไม่มี comma (คั่นด้วยช่องว่างก็ได้)
...
// เขียน updateData ปกติ แต่แยกคอลัมน์ AA (นาทีสะสม — ตัวเลขล้วน) ออกไปเขียนด้วย
// valueInputOption: 'RAW' ต่างหาก — ถ้าใช้ USER_ENTERED เขียนตัวเลขซ้ำๆ ทับกัน Sheets จะ
// auto-detect แล้วล็อก number format ของเซลล์นั้นเป็น Date ค้างไว้ถาวร (ต้นเหตุ bug ตัวเลข
// นาทีสะสมกลายเป็นวันที่ประหลาดอย่าง "23/3/1902" ที่เจอในชีตจริง) ส่วนฟิลด์อื่น (สถานะ/
// หมายเหตุ/เวลาแบบ string) ยังต้องใช้ USER_ENTERED ตามเดิมเพื่อให้ Sheets จัดรูปแบบให้เอง
async function writeRepairUpdate(updateData) {
  const aaData   = updateData.filter(d => d.range.includes('!AA'));
  const restData = updateData.filter(d => !d.range.includes('!AA'));

  if (restData.length) {
    await sheets.spreadsheets.values.batchUpdate({
      spreadsheetId: SPREADSHEET_ID,
      requestBody: { valueInputOption: 'USER_ENTERED', data: restData },
    });
  }
  if (aaData.length) {
    await sheets.spreadsheets.values.batchUpdate({
      spreadsheetId: SPREADSHEET_ID,
      requestBody: { valueInputOption: 'RAW', data: aaData },
    });
  }
}
