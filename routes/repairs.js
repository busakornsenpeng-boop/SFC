...
async function getAllRepairs() {
  ...
  return rows.map(row => ({
    ...
    // หมายเหตุ: column M (กำหนดเสร็จ (เดิม)) เลิกใช้แล้ว — ไม่มีที่ไหนอ้างอิง field นี้อีก
    // ปล่อยคอลัมน์ M ในชีตว่างไว้เฉยๆ ไม่ได้ลบออกจริง เพื่อไม่กระทบ index คอลัมน์ N เป็นต้นไป
    note:         row[13] || '',
    qcResult:     row[14] || '',
    qcBy:         row[15] || '',
    qcNote:       row[16] || '',
