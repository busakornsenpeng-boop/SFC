...
// downtimeSum/downtimeCount ใช้ downtime แบบ "รวม" (แจ้งซ่อม→ปิดงาน) เหมือนที่การ์ดงาน/Excel ใช้
// ไม่หักเวลารออะไหล่ออก — ไม่จำเป็นต้องหัก เพราะมีป้าย "เคยรออะไหล่" กำกับแยกไว้อยู่แล้ว (ดู hadWait)
function calculateTechPerformance(jobs){const m={};jobs.forEach(j=>{if(!j.technician)return;if(!m[j.technician])m[j.technician]={name:j.technician,total:0,done:0,closed:0,back:0,downtimeSum:0,downtimeCount:0};const t=m[j.technician];t.total++;if(j.status==='ซ่อมเสร็จแล้ว')t.done++;else if(j.status==='ปิดงาน'){t.closed++;t.done++;}else if(j.status==='แก้ไข (ตีกลับ)')t.back++;const dt=computeDowntimeMinutes(j);if(dt!=null){t.downtimeSum+=dt;t.downtimeCount++;}});return Object.values(m).map(t=>{const pc=t.total>0?(t.closed/t.total)*100:0;const pb=t.total>0?(t.back/t.total)*100:0;const perf=Math.round(pc*0.6+(100-pb)*0.4);const avgDowntimeMins=t.downtimeCount>0?Math.round(t.downtimeSum/t.downtimeCount):null;return{...t,perfScore:Math.min(100,Math.max(0,perf)),avgDowntimeMins,avgDowntimeText:avgDowntimeMins!=null?formatDurationHM(avgDowntimeMins):'—'};}).sort((a,b)=>b.perfScore-a.perfScore);}
...
function initAdminDashboard(){
  ...
  const lb=sv('leaderboard-tbody');if(lb){lb.innerHTML='';calculateTechPerformance(filtered).forEach((t,i)=>{const rankCls=['rank-1','rank-2','rank-3'][i]||'rank-n';const slaNum=t.total>0?Math.min(100,Math.round(t.perfScore*0.9+10)):null;const slaText=slaNum!==null?slaNum+'%':'—';const slaColor=slaNum>=80?'#10b981':slaNum>=60?'#f59e0b':'#ef4444';const dtColor=t.avgDowntimeMins==null?'var(--text3)':t.avgDowntimeMins<=240?'#10b981':t.avgDowntimeMins<=1440?'#f59e0b':'#ef4444';const tr=document.createElement('tr');tr.innerHTML=`<td><span class="rank-badge ${rankCls}">${i+1}</span></td><td style="font-weight:600;color:var(--text)">${t.name}</td><td style="color:var(--text2);text-align:center">${t.total}</td><td style="color:var(--text2);text-align:center">${t.done}</td><td style="color:#10b981;font-weight:600;text-align:center">${t.closed}</td><td style="color:#ef4444;text-align:center">${t.back}</td><td style="font-family:var(--font-mono);font-weight:600;color:${dtColor};text-align:center;white-space:nowrap">${t.avgDowntimeText}</td><td style="font-family:var(--font-mono);font-weight:700;color:${slaColor};text-align:center">${slaText}</td><td><div style="display:flex;align-items:center;gap:10px"><div style="flex:1"><div class="perf-bar-wrap"><div class="perf-bar-fill" style="width:${t.perfScore}%"></div></div></div><span style="font-family:var(--font-mono);font-size:13px;font-weight:700;color:var(--teal);min-width:36px;text-align:right">${t.perfScore}<span style="font-size:10px;color:var(--text3)">/100</span></span></div></td>`;lb.appendChild(tr);});}
}
